...
Every brain page has up to four zones, separated by horizontal rules (`---`):

**Zone 1 — Frontmatter.** YAML metadata including type, title, tags, and relation arrays (slug paths). Frontmatter is authoritative for relationships — editing these arrays is how users modify connections between entities.

**Zone 2 — Compiled Truth.** Always current, always rewritten when new information arrives. Starts with a one-paragraph executive summary. If you read only this, you know the state of play. Followed by structured State fields, Open Threads (active items — removed when resolved), and subsections appropriate to the entity type.

**Zone 3 — Relationships.** Auto-generated by `gbrain sync` and `gbrain import` from frontmatter relation arrays. Renders as clickable markdown links (`[Title](path.md)`) for navigation in any markdown viewer. Never hand-edit — changes are overwritten on the next sync. Omitted entirely for pages with no relations (backwards compatible two-zone format).

**Zone 4 — Timeline.** Append-only, never rewritten. Reverse-chronological evidence log. Each entry: date, source, what happened. When an open thread gets resolved, it moves here with its resolution.

If someone asks "what's the current state?" — read compiled truth. If someone asks "what happened?" — read timeline. If someone asks "what's connected?" — read the relationships zone or query frontmatter.
...
- **Related Projects:** [Widget Launch](projects/widget-launch.md)
- **Supers:** [Their Manager](people/their-manager.md)
...
## Relationships
- **People:** [Alice Smith](people/alice-smith.md)
- **Related Projects:** [Widget Launch](projects/widget-launch.md)
- **Manager:** [CEO Jane](people/ceo-jane.md)
...
### Other types — same four-zone pattern.

Compiled truth above the first `---`, auto-generated relationships zone between
the two `---` separators, timeline below the second `---`. Pages without
frontmatter relations use the classic two-zone format (no relationships zone).

The relationships zone is generated by `gbrain sync` and `gbrain import` from
frontmatter relation arrays. It renders clickable markdown links for navigation.
Users modify relations by editing frontmatter — never the zone itself.
