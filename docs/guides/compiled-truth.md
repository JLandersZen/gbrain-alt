...
| Compiled truth | After frontmatter, before `<!-- relationships -->` or `<!-- timeline -->` | User / agent | Yes |
| Relationships | Between `<!-- relationships -->` and `<!-- timeline -->` sentinels | gbrain (generated) | No |
| Timeline | After `<!-- timeline -->` sentinel | User / agent | Append only |
...
4. **The sentinel markers matter.** GBrain uses `<!-- relationships -->` and
   `<!-- timeline -->` sentinels after frontmatter to split zones. Pages without
   sentinels fall back to heading-based heuristics (`## Timeline` / `## History`)
   for backwards compatibility with legacy pages.
...
5. **Check relationships zone.** Import a page with frontmatter relations. Verify
   the relationships zone appears with clickable markdown links. Edit the frontmatter
   to add a relation, re-import, verify the zone updates.
