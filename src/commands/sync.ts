...
export async function performSync(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  if (!opts.noExtract && pagesAffected.length > 0) {
    try {
      ...
      const linksCreated = await extractLinksForSlugs(engine, fileBase, pagesAffected);
      const timelineCreated = await extractTimelineForSlugs(engine, fileBase, pagesAffected);
