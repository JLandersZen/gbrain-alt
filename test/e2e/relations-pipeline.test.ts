...
/** Create a temp directory with markdown files that have cross-references */
...
describeE2E('E2E: Relations Pipeline', () => {
  ...
  test('import populates links table from frontmatter relations', async () => {
    ...
    // Import in dependency order: targets must exist before sources referencing them.
    // addLink silently skips when the target page doesn't exist.
    const files = [
      'contexts/at-work.md',
    ...
    // Verify links table for tasks/implement-api
  ...
  test('import populates parent links with type-aware link types', async () => {
    ...
    // aors/engineering has assigned_contexts → links table should have assigned_context
  ...
  test('backlinks (get_backlinks) work for populated links', async () => {
    const engine = getEngine();

    // projects/alpha should be linked TO by tasks/implement-api and tasks/write-tests
  ...
  test('relationships zone is generated on disk', async () => {
    // Read the task file back from disk — importFromFile should have written a zone
  ...
  test('relationships zone preserves four-zone structure', async () => {
    // projects/alpha had compiled truth + timeline already
  ...
  test('relationships zone is idempotent on re-import', async () => {
    ...
    // Read file content after first import
    const contentBefore = readFileSync(join(testDir, 'tasks/implement-api.md'), 'utf-8');

    // Re-import the same file (will be skipped due to hash match if content unchanged)
    // Force re-import by adding a tiny whitespace change
  ...
  test('reverse links are reconstructed across pages', async () => {
    // Collect all page frontmatters for reverse-link reconstruction
    ...
    // Project Alpha should get reverse-link "tasks" field from the two tasks
    ...
    // Alice should get reverse-link related_tasks and related_projects
  ...
  test('reverse link patches applied to disk produce valid four-zone files', async () => {
    ...
    // Apply reverse-link patches (mimics what runImport does post-import)
    const allPages: { slug: string; frontmatter: Record<string, unknown> }[] = [];
    const files = [
      'contexts/at-work.md', 'aors/engineering.md', 'projects/alpha.md',
      'tasks/implement-api.md', 'tasks/write-tests.md',
    ...
    for (const change of changes) {
      ...
      // Re-import so DB reflects the patches
      await importFromFile(engine, filePath, change.slug + '.md', { noEmbed: true, titleMap });
    }

    // Verify Project Alpha now has tasks listed in relationships zone
    ...
    // Verify the file still parses correctly as four-zone
  ...
  test('reverse links update the links table correctly', async () => {
    ...
    // After reverse-link patches + re-import, Project Alpha should have
    // reverse links back to tasks in the links table (as has_member or similar)
    const alphaLinks = await engine.getLinks('projects/alpha');
    const linkTypes = alphaLinks.map((l: any) => l.link_type);
    // projects/alpha should have: assigned_aor (to aors/engineering) + related_person (to people/alice)
    expect(linkTypes).toContain('assigned_aor');
    expect(linkTypes).toContain('related_person');
  });

  test('pages without relations have no relationships zone (two-zone format)', async () => {
    const engine = getEngine();

    // Create a page with no relations at all
    ...
    // Rebuild titleMap to include the new file
    titleMap = buildTestTitleMap(testDir);
  ...
  test('links: nesting (Notion format) is handled correctly', async () => {
    ...
    // Create a file with Notion-style links: nesting
    ...
    // Verify links table was populated despite the nesting
    ...
    // Verify relationships zone was generated
  ...
  test('stale links are removed when relations change', async () => {
    ...
    // Modify tasks/implement-api to remove bob from related_people
    ...
    // Force re-import by changing content hash (remove bob reference)
    const result = await importFromFile(
      engine, filePath, 'tasks/implement-api.md', { noEmbed: true, titleMap },
    );
    expect(result.status).toBe('imported');

    // Verify bob is no longer in the links table for this task
  ...
  test('graph traversal works with relation links', async () => {
    ...
    // Traverse from tasks/implement-api — should find projects/alpha
...
describeE2E('E2E: Sync Pipeline with Relations', () => {
  ...
  test('sync populates links table and generates relationships zone', async () => {
    ...
    // Verify relationships zone was generated on disk
  ...
  test('second sync is idempotent — no spurious changes', async () => {
    const { performSync } = await import('../../src/commands/sync.ts');
    const engine = getEngine();

    // Read file content before second sync
    const before = readFileSync(join(repoPath, 'tasks/sync-task.md'), 'utf-8');

    // Commit the zone changes from first sync so git sees them
